'use client';
import { mosyPostFormData, mosyGetData, mosyUrlParam, mosyUpdateUrlParam , deleteUrlParam, magicRandomStr } from '../../../MosyUtils/hiveUtils';

import { MosyNotify , closeMosyModal, MosyAlertCard } from '@/app/MosyUtils/ActionModals';
...
export async function insertFuelstock() {
 //console.log(`Form fuel_inventory insert sent `)
  ...
  return await mosyPostFormData({
    ...
    url: '/api/octaneorbit/fuelstock/fuelstock',
    method: 'POST',
    isMultipart: true,
  });
}
...
export async function initFuelstockProfileData(rawQstr) {
  ...
  try {
    ...
    const response = await mosyGetData({
      endpoint: '/api/octaneorbit/fuelstock/fuelstock',
      params: { 
      q: btoa(rawQstr),         
    ...
    if (response.status === 'success') {
      //console.log('fuelstock Data:', response.data);  // Process the data
      ...
      console.log('Error fetching fuelstock data:', response.message);  // Handle error
...
export async function getFuelstockListData(qstr = "") {
  ...
  try {
    ...
    if (response.status === 'success') {
      //console.log('fuelstock Data:', response.data);
      return response.data; // ✅ Return the data
    } else {
      console.log('Error fetching fuelstock data:', response);
...
export async function DeleteFuelstock(token = '') {
    ...
    try {
      ...
      const response = await mosyGetData({
        endpoint: '/api/octaneorbit/fuelstock/delete',
        params: { 
...
export async function loadFuelstockListData(customQueryStr, setters) {
    ...
    setters.setFuelstockListData(fuelstockListData)
...
export function popDeleteDialog(deleteToken, setters, router, afterDeleteUrl='../fuelstock/fuelstock')
{     
